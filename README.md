...
- **Webhook de leads** : `LEAD_WEBHOOK` dans `main.js` pointe vers le workflow n8n « VocalFlow — Leads démo partenaires (page marque blanche) »
  ...
  email à contact@vocal-flow.fr. Tous les leads vont dans le Sheet ; la colonne Bouncer vaut ✅ (boîte confirmée), ❌ (refusée par le serveur ou jetable) ou ❔ (Bouncer n'a pas pu conclure : délai, panne, clé absente), et l'alerte reprend ce verdict. Une source commençant par « test » ignore le dédoublonnage (pour rejouer une adresse). L'origine n8n est autorisée dans `connect-src` (`vercel.json`) ; changer d'URL impose de mettre à jour les deux.
